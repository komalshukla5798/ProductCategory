...
function ClearForm(){
  ...
  $('#AddProductFrm input').removeClass('error');
  $('#category_id').val('').trigger('change');
  $('#image_preview').attr('src',default_image);
  $('#image_preview').closest('a').attr('href',default_image);
  $('label.error').hide();
  $('button').attr('disabled',false);
}
...
function readURL(input) {
  if (input.files && input.files[0]) {
    if(input.files[0].size > ini_limit.replace('M','000000') || input.files[0].size > 2000000){ //Size above 2MB restricted
      toastr.error('File is too large');
      return;
    }
    var reader = new FileReader();
    reader.onload = function (e) {
      ReplaceImage(e.target.result); 
      $('#image-error').hide();
    }
...
function ReplaceImage(img){
  if($('#image_preview').closest('a').length == 0){
    $('#image_preview').attr('src',img).attr('width','200px').wrap('<a data-fancybox="gallery" href="'+img+'">');
  }else{
    $('#image_preview').attr('src',img);
    $('#image_preview').closest('a').attr('href',img); 
  }
}
